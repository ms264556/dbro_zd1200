...
#   * BOOTFS     — the boot-area inputs (grub097_src/out or build-bootfs.py) changed
...
BOOTFS_SRC_DIR="$BASE/grub097_src/out"
...
[ -d "$BOOTFS_SRC_DIR" ] || { echo "apply-rootfs-patches: missing GRUB artifact dir: $BOOTFS_SRC_DIR (run grub097_src/build.sh)" >&2; exit 1; }
...
# grub097_src/out by build-bootfs.py), so a change to either means the base must
# be rebuilt (and the overlay re-patched).
rootfs_sig="$(sha256sum "$ROOTFS" | awk '{print $1}')"
bootfs_sig="$( cd "$BASE" && { find grub097_src/out -type f -print | LC_ALL=C sort | xargs sha256sum; sha256sum build-bootfs.py; } | sha256sum | awk '{print $1}')"
