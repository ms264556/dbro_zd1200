...
# MAC-bearing fields, relative to each record's start, paired with which of the
# two MACs the builder writes there.  Kept next to the builders below so the two
# cannot drift apart: every field the builders write from mac1/mac2 appears here,
# and nothing else.  The pattern is NOT alternating -- in the ar531x record both
# wlan0 and enet0 carry MAC1 while both enet1 and wlan1 carry MAC2 -- so each
# field names its source explicitly rather than relying on parity.
#
# `--mac-only` rewrites only these, which is what lets the container re-assert
# its own MAC before every guest boot against a guest that changed it (the
# vendor's rbd.sh / bsp_commit write the same fields) without touching the
# serial, model, customer or certificates the operator may have set.
AR531X_MAC_FIELDS = ((0x60, 1), (0x66, 1), (0x6C, 2), (0x76, 2))  # wlan0, enet0, enet1, wlan1
RKS_MAC_FIELDS = ((0x58, 1), (0x5E, 2), (0x7E, 1))                # enetxMac[0], [1], MACbase


def apply_mac(record: bytearray, fields, mac1: bytes, mac2: bytes) -> None:
    """Overwrite the MAC fields of an existing board-data record in place.

    The caller hands in a record already read off the disk, so only the MAC
    fields change and anything else rbd.sh wrote survives.  `fields` is a
    sequence of (offset, which), where which is 1 for MAC1 and 2 for MAC2, and
    must mirror the builder for that record exactly.
    """
    for off, which in fields:
        record[off:off + 6] = mac1 if which == 1 else mac2
...
def main():
    ...
    ap.add_argument("--mac-only", action="store_true",
                    help="rewrite only the MAC fields of the records already on "
                         "the disk, keeping the serial/model/customer they carry "
                         "(and recomputing rks_boarddata.cksum).  Used before "
                         "every guest launch so the container's MAC always wins "
                         "over a MAC a guest set with rbd.sh.")
    args = ap.parse_args()
    ...
    if args.mac_only:
        # Read-modify-write: the records are already on the disk, so the serial,
        # model and customer (possibly rewritten by rbd.sh in the guest) are
        # preserved and only the MACs are forced back to ours.
        with disk.open("r+b") as f:
            f.seek(ar531x_off)
            abd = bytearray(f.read(AR531X_STRUCT_SIZE))
            f.seek(rks_off)
            rbd = bytearray(f.read(RKS_STRUCT_SIZE))
            if abd[:4] != struct.pack("<I", AR531X_BD_MAGIC):
                raise SystemExit(
                    f"{disk}: no ar531x_boarddata magic at 0x{ar531x_off:x} "
                    f"(found {bytes(abd[:4])!r}); refusing to write")
            if rbd[:4] != struct.pack("<I", RKS_BD_MAGIC):
                raise SystemExit(
                    f"{disk}: no rks_boarddata magic at 0x{rks_off:x} "
                    f"(found {bytes(rbd[:4])!r}); refusing to write")
            apply_mac(abd, AR531X_MAC_FIELDS, mac1, mac2)
            apply_mac(rbd, RKS_MAC_FIELDS, mac1, mac2)
            struct.pack_into("<H", rbd, 0x04, rks_cksum(bytes(rbd)))
            f.seek(ar531x_off)
            f.write(abd)
            f.seek(rks_off)
            f.write(rbd)
        print(f"{disk}: MAC1 {mac1.hex(':')} MAC2 {mac2.hex(':')} "
              f"(mac-only; serial/model/customer preserved)")
        return
