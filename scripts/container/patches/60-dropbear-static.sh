...
# the static musl build (ms264556/zd_dropbear, vendored under
# packages/dropbear/) and add a public-key-only root SSH listener on TCP 2222.
#
...
# (/opt/zd1200/packages/dropbear/dropbear, only present when ZD_ROOT_SSH=1) and a
# public key is supplied.  When the payload is absent it REVERTS an earlier
# install, so turning the feature off in install-zd1200-docker.sh restores the
# vendor binary.
#
...
install_payload() {
    ...
    if [ -n "$(fs_stat_meta "$img" /usr/sbin/dropbear)" ] \
       && [ -z "$(fs_stat_meta "$img" /usr/sbin/dropbear.vendor)" ]; then
    ...
    if [ -z "$(fs_stat_meta "$img" /.ssh)" ]; then
...
revert_payload() {
    ...
    if [ -n "$(fs_stat_meta "$img" /usr/sbin/dropbear.vendor)" ]; then
    ...
    if [ -n "$(fs_stat_meta "$img" /etc/shells.vendor)" ]; then
    ...
    remove_path "$img" /usr/sbin/dropbear.vendor
    remove_path "$img" /etc/shells.vendor
    remove_path "$img" /usr/sbin/sftp-server
    remove_path "$img" /etc/zd1200-root-authorized_keys
