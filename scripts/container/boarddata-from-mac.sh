...
# Guest MAC1 is the seed itself.  On Docker the seed is a synthesised identity
# (not an interface address); on Proxmox it is the container's own uplink MAC
# when the installer shares it, which is exactly the identity the LAN should see.
mac="$container_mac"
