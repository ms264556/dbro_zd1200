#!/usr/bin/env python3
"""Unit-test scripts/container/proxmox/zd1200-ct-address without touching real interfaces.
...
HELPER = os.path.join(REPO, "scripts", "container", "proxmox", "zd1200-ct-address")
...
if a[:2] == ["addr", "flush"]:
    ...
    save(s)
    sys.exit(0)
if a[:2] == ["link", "set"] and "address" in a:
    s.setdefault("mac", {})[a[2]] = a[a.index("address") + 1]
    save(s)
    sys.exit(0)
if a[:1] == ["route"]:
    if "show" in a:
...
class Fixture:
    ...
    def mac(self, dev):
        try:
            with open(self.ip_state) as fh:
                return json.load(fh).get("mac", {}).get(dev, "")
        except Exception:
            return ""
...
def main():
    with tempfile.TemporaryDirectory(prefix="zd-ct-address-test.") as tmp:
        ...
        # --- the shared-MAC handover -----------------------------------------
        # With ZD_SHARE_UPLINK_MAC=1 the bridge wears the uplink's MAC while the
        # container holds the address and its own distinct MAC while the guest
        # does.  ZD_HOST_IF=lo supplies a readable uplink MAC without a real NIC.
        try:
            with open("/sys/class/net/lo/address") as fh:
                lo_mac = fh.read().strip()
        except OSError:
            lo_mac = ""
        if lo_mac:
            fx.run("down")
            fx.run("up", ZD_SHARE_UPLINK_MAC="1", ZD_HOST_IF="lo")
            check("shared: the container takes the uplink MAC",
                  fx.mac("br-zd") == lo_mac, f"mac={fx.mac('br-zd')} want={lo_mac}")
            fx.run("down", ZD_SHARE_UPLINK_MAC="1", ZD_HOST_IF="lo")
            check("shared: down hands the guest the distinct MAC",
                  fx.mac("br-zd") == "02:00:00:00:00:01", f"mac={fx.mac('br-zd')}")
            fx.run("up", ZD_SHARE_UPLINK_MAC="1", ZD_HOST_IF="lo")
            check("shared: up takes the shared MAC back",
                  fx.mac("br-zd") == lo_mac, f"mac={fx.mac('br-zd')}")
        else:
            print("SKIP: /sys/class/net/lo/address is unreadable")

        # Without sharing the bridge MAC is never touched.
        mac_before = fx.mac("br-zd")
        fx.run("down")
        fx.run("up")
        check("unshared: the bridge MAC is left alone",
              fx.mac("br-zd") == mac_before, f"mac={fx.mac('br-zd')} before={mac_before}")
