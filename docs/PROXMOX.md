...
the guest joins your LAN as an ordinary Layer-2 device wearing the container's
own MAC and taking its own DHCP lease — reachable from the Proxmox host, from the
container, and from any other machine on the LAN.
...
./install-zd1200-lxc.sh \
    ...
    /root/zd-inputs/zd1200_10.5.1.0.282.ap_10.5.1.0.282.img \
    --root-ssh-key ~/.ssh/id_ed25519.pub \
    --yes --non-interactive
```

`./install-zd1200-lxc.sh --help` lists every flag. The inputs are positional (a
firmware upgrade file, a configuration backup, or a card dump, and the firmware
as a second argument when the first is a backup or a foreign dump; the
`--source`, `--backup` and `--writable-from` names are aliases), and `--no-*`
switches turn off the optional pieces.
...
- `zd1200-console-tty.service` runs `scripts/container/proxmox/zd1200-console-bridge.py`
  with `TTYPath=/dev/tty1`, relaying between that tty and QEMU's console chardev.
...
- `zd1200.service` runs `zd1200-ct-address down` just before it starts QEMU, and
  `up` again when QEMU exits without the appliance being switched off (a failed or
  restarted guest). A guest poweroff shuts the container down with it.
...
This reboot comes back on the same root: it is the guest kernel's own watchdog
marker (`'9'`) that moves the boot entry to the spare root (see
`docs/INTERNALS.md`).
...
| start after the appliance powered itself off | `pct start <id>` (that also stopped the container) |
...
└── LXC container (Debian)
    ├── eth0 ── tc cross-connect ──┐    (carries the container's MAC; no address)
    ├── br-zd ─────────────────────┘    (container's own address lives here)
    │   └── tap-zd ──────── QEMU guest  (wears the same MAC + DHCP lease on the LAN)
    ...
    ├── /opt/zd1200                     repo checkout (scripts, packages, patches)
...
- `scripts/container/proxmox/zd1200-ct-bootstrap.sh` — runs **inside** the CT:
  ...
  systemd units.
- `scripts/container/proxmox/zd1200-ct-net.sh` — runs inside the CT as
  `zd1200-net.service`: bridges the uplink, moves the CT's address onto the
  bridge, and creates the display interface ahead of it.
- `scripts/container/proxmox/zd1200-guest-display` — keeps the guest's
  (lease-derived) address on the display interface so the Proxmox Summary shows
  it.
- `scripts/container/proxmox/zd1200-console-bridge.py` — runs inside the CT as
  ...
  above).
- `scripts/container/proxmox/zd1200-ct-address` — holds or releases the
  container's own LAN address. `zd1200.service` calls it around QEMU, so only the
  guest has an address while the appliance runs (see "The container's own
  address" above).
- `scripts/build/proxmox/zd1200-pve-summary-host.sh` — runs on the PVE host as a
  timer: writes each guest's address into its container's Summary description.
...
**board data**, and the LAN identifies the appliance by it. On Proxmox the guest
wears the container's own uplink MAC, so the LAN sees exactly the identity
Proxmox allocated and no invented MAC is on the wire:

| MAC | what it is | rule |
|---|---|---|
| container uplink (`eth0`) | the veth PVE creates | the shared identity; the guest wears it |
| container bridge (`br-zd`) | the CT's own address lives here | forced distinct (`02:00:00:00:00:01`) |
| guest board MAC1 | what the guest transmits | the container's uplink MAC |

The installer reads the MAC Proxmox allocated for `net0` and builds the guest's
board-data MAC1 from it; `--container-mac M` pins the CT's `net0 hwaddr` to `M`
instead. Before every launch `launch-vm.sh` re-asserts that MAC in the board
data, so a MAC changed inside the guest is reverted on the next boot, and the
`35-rbd-mac-guard.sh` rootfs patch refuses a `rbd.sh` MAC change outright.

The MAC is held by whichever side is up. While the guest runs the container
releases its own address (`ZD_CT_ADDRESS_FOLLOW_QEMU=1`, the default) and its
bridge moves to its distinct MAC, so only the guest answers as `X`. When the
guest stops the container takes the address back *and* moves its bridge back to
`X` first, so the LAN sees the same identity (and the DHCP server hands back the
same lease) whether the appliance is running or not. Change `net0`'s `hwaddr` in
Proxmox and restart the container, and the guest adopts the new MAC on its next
boot.

**Why the uplink is not a bridge port.** A Linux bridge installs a permanent
local FDB entry for every port's *own* MAC and delivers frames addressed to it
to that port. With the uplink enslaved, a guest wearing the uplink's MAC never
sees its DHCP reply — the frame goes to the addressless uplink instead of the
tap, so the guest loops on DISCOVER and sits on its `192.168.0.2` fallback
(observed on PVE 9.2 with a Debian 13 CT). `zd1200-ct-net.sh` therefore
cross-connects the uplink to `br-zd` with a veth pair and a two-way `tc` redirect:
an L2 patch lead with no MAC learning and no local entry to shadow anything. The
container and the guest can still reach each other, and so can the Proxmox host.

The container bridge's own MAC is also moved off the shared one
(`02:00:00:00:00:01` by default, `ZD_BRIDGE_MAC_ADDRESS` to change) so the
container's address and the guest's never collide.
