...
#   ./install-zd1200-lxc.sh firmware.img               # install from firmware alone
#   ./install-zd1200-lxc.sh backup.bak firmware.img    # configuration backup + firmware
#   ./install-zd1200-lxc.sh cfcard_dump.img            # a ZD1200 card dump (self-contained)
#   ./install-zd1200-lxc.sh foreign_dump.bin firmware.img
#                                                      # a ZD1100/ZD3000 dump + firmware
#   ./install-zd1200-lxc.sh --yes --ctid 120 ...       # unattended
#
...
# (e.g. 9.10.2.0.84 pairs with 9.10.2.0.130).
#
# Options:
...
#                            Takes no input.  Uses --ctid to pick the container,
#                            or finds the one this installer made.
#   --writable-from PATH     named alias for a dump positional argument: take
#                            /writable + serial from the dump while the
#                            kernel/rootfs come from the firmware
#   --writable-partition S:C override the detected dump geometry
#   --backup PATH            named alias for a configuration-backup positional
#                            argument: the guest runs the vendor restore on its
#                            first boot and reboots once
#   --container-mac MAC      pin the container's own MAC (the net0 hwaddr).  With
#                            the default shared MAC the guest wears this same
#                            value, so the LAN sees exactly the MAC you choose;
#                            without --share-mac the guest gets its own MAC
#   --share-mac / --no-shared-mac
#                            whether the guest wears the container's own MAC
#                            (default: yes — the LAN sees the identity Proxmox
#                            allocated and no invented MAC is on the wire).
#                            --no-shared-mac keeps the old rule: a fresh random
#                            guest MAC, distinct from the container's
...
POSITIONALS=(); OPT_FIRMWARE=""; OPT_BACKUP=""; OPT_WRITABLE=""; WRITABLE_PARTITION=""; CONTAINER_MAC_OVERRIDE=""
ADVANCED=0
ROOT_SSH_KEY=""; ECDSA=1; NETWORK_MONITOR=1; R600_REPAIR=1; CONSOLE_TTY=1; KEEP_CT_ADDRESS=0
SHARE_UPLINK_MAC=1; SHARE_UPLINK_MAC_SET=0
...
list_sources() {
    ...
    for d in "${dirs[@]}"; do
        ...
        find "$d" -maxdepth 1 -type f \
             \( -name '*.img' -o -name '*.bin' -o -name '*.bak' -o -name '*.tgz' \
                -o -name '*.7z' -o -name '*.zip' \) 2>/dev/null
    done | sort -u
}
...
upgrade_existing_container() {
    ...
    # An upgrade keeps the identity the container already has: an existing
    # install has no ZD_SHARE_UPLINK_MAC (it predates sharing), so it stays on
    # its current distinct-MAC identity unless --share-mac is passed explicitly.
    if [ "$SHARE_UPLINK_MAC_SET" = 0 ]; then
        v="$(ct_conf_get "$CTID" ZD_SHARE_UPLINK_MAC)"
        if [ "$v" = "1" ]; then SHARE_UPLINK_MAC=1; else SHARE_UPLINK_MAC=0; fi
    fi
    ...
    # (packages/dropbear/, ruckus-squashfs/) are deliberately kept; the bootstrap
    # rebuilds packages/analytics and re-links
    # scripts/container/{image,packages,ruckus-squashfs}.  The bare analytics/
    # bl7/ dropbear/ proxmox/ names are the pre-packages/ layout, removed so an
    # upgrade from it does not leave two copies of the payloads behind.
    pct exec "$CTID" -- bash -c "cd '$DEFAULT_CT_REPO' && rm -rf analytics bl7 dropbear proxmox packages/analytics packages/mesh-patch docker docs scripts && rm -f README.md LICENSE install-zd1200-docker.sh install-zd1200-lxc.sh"
    tar -C "$REPO_ROOT" -cf - \
    ...
    if ! pct exec "$CTID" -- "$DEFAULT_CT_REPO/scripts/container/proxmox/zd1200-ct-bootstrap.sh" "${bootstrap_args[@]}"; then
...
# 1. input selection
# --------------------------------------------------------------------------
# The primary argument is a configuration backup or a card dump — or a firmware
# upgrade file, for a bare install.  When the primary is a backup or a
# non-ZD1200 dump, the matching firmware is the second argument.  --source,
# --backup and --writable-from remain accepted as named aliases.
if [ "${#POSITIONALS[@]}" -eq 0 ] && [ -z "$OPT_FIRMWARE$OPT_BACKUP$OPT_WRITABLE" ]; then
    mapfile -t FOUND < <(list_sources)
    if [ "$INTERACTIVE" = 0 ]; then
        die "an input is required in --non-interactive mode: a firmware, a backup or a card dump"
    fi
    choices=()
    for f in "${FOUND[@]}"; do
        choices+=("$f" "$(classify_config_input "$f") — $(( $(stat -c%s "$f") / 1048576 )) MiB")
    done
    choices+=("other" "Enter the path to a firmware, backup or card dump")
    choices+=("advanced" "Advanced: choose container id, storage, sizing and features")
    picked="$("${WT[@]}" --menu "Which firmware, backup or card dump should it build from?" 22 96 12 "${choices[@]}" 3>&1 1>&2 2>&3)" \
        || die "cancelled"
    case "$picked" in
        other|advanced)
            [ "$picked" = advanced ] && ADVANCED=1
            POSITIONALS+=("$("${WT[@]}" --inputbox "Path to the firmware upgrade file, configuration backup or card dump:" 10 90 "" 3>&1 1>&2 2>&3)") || die "cancelled"
            ;;
        *)  POSITIONALS+=("$picked") ;;
    esac
fi

# Apply the input rules: one backup-or-dump, an optional firmware, and the
# documented errors when they do not fit (scripts/install-common.sh).
resolve_inputs

for f in "$INPUT_PATH" "$FIRMWARE_PATH"; do
    [ -n "$f" ] || continue
...
if [ -n "$WRITABLE_PARTITION" ]; then
    case "$INPUT_KIND" in
        ...
        *) die "--writable-partition is only meaningful with a card dump input" ;;
    esac
fi

case "$INPUT_KIND" in
    firmware-only) info "input: $FIRMWARE_PATH (firmware upgrade image)" ;;
    backup)        info "input: $INPUT_PATH (configuration backup) + firmware $FIRMWARE_PATH" ;;
    zd1200-dump)   info "input: $INPUT_PATH (ZD1200 card dump)$([ -n "$FIRMWARE_PATH" ] && printf ' + firmware %s' "$FIRMWARE_PATH")" ;;
    foreign-dump)  info "input: $INPUT_PATH (foreign card dump) + firmware $FIRMWARE_PATH" ;;
esac
...
# 2b. identity
# --------------------------------------------------------------------------
# Proxmox allocates the container's veth MAC itself (PVE::Tools::random_ether_addr
# with the cluster's mac_prefix) and the container keeps it.  The guest is given
# a separate random MAC from the same function, with its last octet cleared --
# see section 4 and guest_mac_seed() in the bootstrap.
# --------------------------------------------------------------------------
...
Container
  ...
  guest MAC ......... $([ "$SHARE_UPLINK_MAC" = 1 ] && printf "the CT's own (shared)%s" "${CONTAINER_MAC_OVERRIDE:+ ($CONTAINER_MAC_OVERRIDE)}" || echo "its own random MAC (not shared)")
...
Input
  kind .............. $INPUT_KIND
$([ -n "$INPUT_PATH" ] && printf '  backup/dump ....... %s\n' "$INPUT_PATH")
$([ -n "$FIRMWARE_PATH" ] && printf '  firmware .......... %s\n' "$FIRMWARE_PATH")
...
# --------------------------------------------------------------------------
# Let Proxmox allocate the container's MAC (cluster-aware, unique) unless the
# operator pinned one with --container-mac.  That MAC is the identity the LAN
# will see: with the default shared MAC the guest wears it too (the container's
# network setup cross-connects the uplink rather than enslaving it, so the
# shared value is not shadowed by a bridge port's own FDB entry).
net_spec="name=eth0,bridge=$BRIDGE,firewall=0,type=veth"
[ -n "$CONTAINER_MAC_OVERRIDE" ] && net_spec+=",hwaddr=$CONTAINER_MAC_OVERRIDE"
if [ -n "$STATIC_IP" ]; then
...
# The guest's identity.  PVE has now allocated (or the operator pinned) the
# container's own MAC; read it, and either share it or draw a separate one.
#
#   * shared (the default): the guest wears the container's MAC, so the LAN sees
#     exactly the identity Proxmox allocated and no invented MAC is on the wire.
#     The container's network setup cross-connects the uplink to its bridge
#     instead of enslaving it, so a bridge port's own-MAC FDB entry cannot shadow
#     the guest (verified on PVE 9.2).
#   * --no-shared-mac: the old rule, a fresh random MAC from Proxmox's
#     random-MAC function with the last octet's low bit cleared (so MAC2 =
#     MAC1 + 1 stays an odd number in the same prefix).  It must differ from the
#     container's, or a guest wearing a bridge port's own MAC never sees its DHCP
#     reply and sits on its 192.168.0.2 fallback.
GUEST_MAC=""
ct_mac="$(pct config "$CTID" | sed -n 's/^net0:.*hwaddr=\([0-9A-Fa-f:]*\).*/\1/p' | head -1 | tr 'A-Z' 'a-z')"
if [ "$SHARE_UPLINK_MAC" = 1 ]; then
    ...
    if [ -n "$mac_prefix" ]; then
        GUEST_MAC="$(perl -MPVE::Tools -e 'print PVE::Tools::random_ether_addr($ARGV[0])' \
            "$mac_prefix" 2>/dev/null | tr 'A-Z' 'a-z')"
    fi
    if is_mac "$GUEST_MAC"; then
        GUEST_MAC="$(printf '%s:%02x' "${GUEST_MAC%:*}" "$(( 0x${GUEST_MAC##*:} & 0xFE ))")"
        [ "$GUEST_MAC" = "$ct_mac" ] && \
            warn "the guest MAC landed on the container's ($ct_mac); pass --container-mac to pick another"
        info "guest MAC: $GUEST_MAC (drawn from Proxmox's random-MAC function, not shared)"
    else
        GUEST_MAC=""
    fi
fi
...
step "staging the input inside the container"
pct exec "$CTID" -- mkdir -p /root/zd-inputs
stage_push() {  # stage_push <host path> <ct variable name>
    local src="$1" var="$2"
    [ -n "$src" ] || return 0
    pct push "$CTID" "$src" "/root/zd-inputs/$(basename "$src")"
    printf -v "$var" '%s' "/root/zd-inputs/$(basename "$src")"
}
# prepare-vendor-image.sh keeps its existing shape: the positional is the
# firmware (or a self-contained ZD1200 dump), --writable-from pairs a dump's
# /writable with the firmware, and --backup stages a configuration backup.
ct_source=""; ct_writable=""; ct_backup=""
case "$INPUT_KIND" in
    firmware-only) ct_source="$FIRMWARE_PATH" ;;
    backup)        ct_source="$FIRMWARE_PATH"; ct_backup="$INPUT_PATH" ;;
    zd1200-dump)
        if [ -n "$FIRMWARE_PATH" ]; then
            ct_source="$FIRMWARE_PATH"; ct_writable="$INPUT_PATH"
        else
            ct_source="$INPUT_PATH"
        fi ;;
    foreign-dump)  ct_source="$FIRMWARE_PATH"; ct_writable="$INPUT_PATH" ;;
esac
stage_push "$ct_source" ct_source
stage_push "$ct_writable" ct_writable
stage_push "$ct_backup" ct_backup

# The guest's identity seed.  When sharing, tell the bootstrap to take the
# container's own uplink MAC (it reads the live interface, so it cannot drift
# from the net0 hwaddr).  Otherwise pass the distinct MAC chosen above.
# --------------------------------------------------------------------------
...
if [ -n "$ct_backup" ]; then
    bootstrap_args+=(--backup "$ct_backup")
else
    # No backup in this run: never let one a previous run staged be picked up.
...
if ! pct exec "$CTID" -- test -x "$DEFAULT_CT_REPO/scripts/container/proxmox/zd1200-ct-bootstrap.sh"; then
    die "the project did not copy into container $CTID correctly (expected $DEFAULT_CT_REPO/scripts/container/proxmox/zd1200-ct-bootstrap.sh)"
fi
if ! pct exec "$CTID" -- "$DEFAULT_CT_REPO/scripts/container/proxmox/zd1200-ct-bootstrap.sh" "${bootstrap_args[@]}"; then
